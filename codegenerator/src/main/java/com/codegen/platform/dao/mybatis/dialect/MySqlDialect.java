package com.codegen.platform.dao.mybatis.dialect;
...
public class MySqlDialect extends Dialect {
	...
	public String getTranslateString(String querySelect, String translateColumn) {
		...
		int formIndex = getAfterFormInsertPoint(querySelect);
		String select = querySelect.substring(0, formIndex);
		if (StringUtils.isNotEmpty(translateColumn)) {
			return new StringBuffer(select).append(",").append(translateColumn).append(querySelect.substring(formIndex))
					.toString();
		}
		return querySelect;
	}
	...
	public String[] getSqlColumn(String querySelect) {
		...
		for (int i = 0; i < columns.length; i++) {
			...
			if (StringUtils.isNotEmpty(column)) {
				column = column.trim();
				if (column.contains(" ")) {
					Integer idx = column.lastIndexOf(" ");
					if (idx > 0) {
						columns[i] = column.substring(idx).trim().replace("a.", "");
					}
				} else {
					columns[i] = column.replace("a.", "");
				}
			}
		}
		return columns;
