package com.cw.stu.internet.tech.platform.dao.database;
...
import com.cw.stu.internet.tech.platform.util.SpringUtils;
...
public class PublicDAOService {
     ...
     * @param objParam  可以使用 DBTableParamBean 或者 Map 类型 要求参数中必须包含现表的主键信息
     ...
     * @param offset   前面跳过多少条记录
     * @param limit    最多取出多少条记录，当limit为0时则offset参数无效并且不限制记录数
     * @return 返回查询结果集
     */
     ...
     * 动态SQL查询，返回多条记录
     *
     * @param sqlText 原则上要求传入返回VARCHAR或者NUMBER的 SQL语句
     ...
     * 动态SQL查询，返回一行一列的记录
     *
     * @param sqlText 原则上要求传入只返回一行一列返回值为VARCHAR或者NUMBER的SQL
     ...
     *
     * @param sqlText 原则上要求传入只返回一列返回值为VARCHAR或者NUMBER的SQL
