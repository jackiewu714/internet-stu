...
public class ListUtil {
     ...
     * 将字符串解析成列表
     * 
     * @param imids IM号字符串，以separator分割
     * @param separator	分隔符
     * @return List<String>
     */
     ...
     * @param list List<String> 对象列表
     * @param separator	分隔符
     * @param includeCharacter 包含每个字符串的字符，例如单引号"'"，包含 123 后变成 '123'
     ...
     * @return List<String> 字符串列表，例如: '123','456','789'
     */
    public static String parseListToStringIncludeByCharacter(List<String> list, String separator, String includeCharacter) {
	StringBuffer sb = new StringBuffer();
	if (list != null && list.size() > 0) {
	    for (int i = 0; i < list.size(); i++) {
		String str = list.get(i) == null ? "" : list.get(i).toString();
